...
int create_display(int displayId)
{
    int ret = 0;
    ...
    for (int i = 0; i < 5 * 1000; ++i)
    {
        ...
        if ((hwcDisplay = hwc2_compat_device_get_display_by_id(hwcDevice, displayId)))
            break;
        usleep(1000);
    }
    assert(hwcDisplay);
    ...
    if (evdi_connect(fd, displayId, config->width, config->height, refresh_hz) < 0)
    {
        close(fd);
        return EXIT_FAILURE;
    }
...
{
    int displayCount = 3;
    std::future<int> ret[displayCount];
    std::thread *displayThread[displayCount]; // 0 for primary display, 1 for external display
    sd_notifyf(0, "MAINPID=%lu", (unsigned long)getpid());
    sd_notify(0, "STATUS=Initializing create-disp…");

    for (int i = 0; i < displayCount; i++)
    {
        std::packaged_task<int(int)> task(create_display);
        ret[i] = task.get_future();
        displayThread[i] = new std::thread(std::move(task), i);
    }

    for (int i = 0; i < displayCount; i++)
    {
        displayThread[i]->join();
    }

    for (int i = 0; i < displayCount; i++)
    {
        int retValue = ret[i].get();
